...
void analyze_segments(char* sample_segment, char** candidate_segments, int number_of_candidates, char* output_string)
{
  ...
  for (i = 0; i < number_of_candidates; i++) {
      ...
      if (check_if_equal(sample_segment, candidate_segments)) {
...
int calculate_score(char* sample_segment, char* candidate_segment)
{
  ...
  int sample_length_in_codons = sample_length / CODON_LENGTH;
  int candidate_length_in_codons = candidate_length / CODON_LENGTH;

  //controls shifting the whole sample over
  while (iterations <= (candidate_length_in_codons - sample_length_in_codons)) {
     
      //comparing the sample to the candidate one codon at a time
      for (i = 0; i < sample_length_in_codons; i++) {
          
          if (are_codons_equal(sample_segment, candidate_segment) == 1) {
              temp_score += 10;
          }

          else if (are_same_amino(sample_segment, candidate_segment) == 1) {
              temp_score += 5;
          }
              ...
              for (j = 0; j < CODON_LENGTH; j++) {
                  ...
                  //a&t, c&g
                  else if (is_base_pair(sample_segment[j], candidate_segment[j])) {
                      temp_score += 1;
                  }
      ...
      iterations++;
      sample_segment -= CODON_LENGTH * sample_length_in_codons;
      candidate_segment -= (CODON_LENGTH * (sample_length_in_codons - 1));
      temp_score = 0;

  }

  candidate_segment -= CODON_LENGTH * candidate_length_in_codons;
...
*
*/
int check_if_equal(char* str1, char** str2)
{
    ...
    if (str1_length == str2_length) {
        if (strncmp(str1, *str2, str1_length) == 0) {
            result = 1;
        }
    }
...
int are_codons_equal(char* str1, char* str2)
{
    int result = 0;
    
    if (strncmp(str1, str2, CODON_LENGTH) == 0) {
            result = 1;
    }
...
//compare codon codes with codon names
//checking to see if 
int are_same_amino(char* cod1, char* cod2)
{
    int index = 0;
    int result = 0;
   
    char amino_1[CODON_NAME_LENGTH];
    char amino_2[CODON_NAME_LENGTH];
    
    char int_buffer[BUFSIZ];
    //scan codon_codes and find matching codon code
    //identify index
    
    index = get_codon_index(cod1);
   // strcat(amino_1, codon_names[index]);
    sprintf(amino_1, "%s", codon_names[index]);

    index = get_codon_index(cod2);
   // strcat(amino_2, codon_names[index]);
    sprintf(amino_2, "%s", codon_names[index]);

    if (strlen(amino_1) == strlen(amino_2)) {
        if (strncmp(amino_1, amino_2, strlen(amino_1)) == 0) {
            result = 1;
        }
    }
    return result;

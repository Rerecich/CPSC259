...
maze_cell** parse_maze( FILE* maze_file, int dimension )
{
  ...
  // maze = ( maze_cell ** ) calloc ... (1 line)
	maze = (maze_cell**)calloc(dimension, dimension);

  for ( row = 0; row < dimension; ++row ) {
    // INSERT CODE HERE (1 line)
    // maze[row] = ( maze_cell* ) calloc ... (1 line)
	  maze[row] = (maze_cell*)calloc(dimension, dimension);
  }
...
void generate_all_paths(char*** pathsetref, int* numpathsref, maze_cell** maze, int dimension, int row, int column, char* path)
{
  ...
  /* Otherwise deals with the recursive case.  Pushes the current coordinate onto the path
	  ...
	  //--------------assigning local path variables-----------//
	  if ( path_length ) {
		  new_path = strcat( new_path, path );
	  }
	  new_path = strcat( new_path, new_point ); //adds current point to the path
	  free(new_point);
	  
	  /*
	  POINT POSITION ASSESSMENT
	  		CASE 1: RIGHT BOUNDARY REACHED
	  ...
	  */
	  ...
	  if ( column == ( dimension - 1 ) ) {
		  //---------------------reallocating memory in global path array-----------------------//
		  *pathsetref = ( char** ) realloc ( *pathsetref, ( (*numpathsref) + 1 ) * sizeof( char* ) );
		  ...
		  //---------------------save this path to global success path-------------------------//
	  ...
	  //************************************CASE 2: CONTINUE SEARCH*****************************************//
	  else {
		  /*  */
      // INSERT CODE HERE (6 lines)
      // maze.[row][column].visited = ...
      // generate_all_paths...up
      // generate_all_paths...down
      // generate_all_paths...left
      // generate_all_paths...right
      // maze.[row][column].visited = ...
		  maze[row][column].visited = VISITED;
...
int path_cost ( char* path_string )
{
  ...
  if (*path_string == '\0') {
	  ...
	  return (*path_string + path_cost(path_string + 1) - '0');
  }
}
...
void construct_shortest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	...
	int count = 10000;
	int temp_count = 0;
	char* shortest = (char*)malloc(sizeof(BUFFER));
	shortest = "";
	...
	sprintf(outputbuffer, "Shortest path: %s", shortest);
	strcat(outputbuffer, "\n");
	printf(outputbuffer);
	
	//free(shortest); this makes it not work
...
void construct_cheapest_path_info ( char** pathset, int numpaths, char* outputbuffer )
{
	...
	char* cheapest = (char*)malloc(sizeof(BUFFER));
	cheapest = "";

	for (i = 0; i < numpaths; i++) {
		cost_temp = path_cost(pathset[i]);
	...
	sprintf(cheapest, "%d", cost);

	strcat(outputbuffer, "Cheapest path cost: ");
	sprintf(cheapest, "%d", cost);
	strcat(outputbuffer, cheapest);
	strcat(outputbuffer, "\n");
	//
